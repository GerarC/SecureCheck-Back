...
public class StreamUtils {
    ...
    public static <A, B> List<B> map(Collection<A> items, Function<A, B> mapper) {
        return items.stream().map(mapper).toList();
    }

    public static <A> A findAny(Collection<A> items, Predicate<A> predicate, RuntimeException exception){
    ...
    public static <A, B> B mapAndReduce(Collection<A> items, B initialValue, Function<A, B> mapper, BinaryOperator<B> reducer){
